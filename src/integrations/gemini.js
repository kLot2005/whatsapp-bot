...
const CONSENT_PROMPT = `Определи: согласился ли клиент записаться на консультацию?
...
// ─── Инициализация моделей (Pre-initialized for efficiency) ──────────────────

const empathyModel = genAI.getGenerativeModel({
    model: 'gemini-2.5-flash',
    generationConfig: { temperature: 0.65, topP: 0.95 },
    systemInstruction: EMPATHY_PROMPT,
});

const digDeeperModel = genAI.getGenerativeModel({
    model: 'gemini-2.5-flash',
    generationConfig: { temperature: 0.65, topP: 0.95 },
    systemInstruction: DIG_DEEPER_PROMPT,
});

const offerModel = genAI.getGenerativeModel({
    model: 'gemini-2.5-flash',
    generationConfig: { temperature: 0.65, topP: 0.95 },
    systemInstruction: OFFER_PROMPT,
});
...
const consentModel = genAI.getGenerativeModel({
    model: 'gemini-2.5-flash',
    generationConfig: { temperature: 0.1, topP: 0.9 },
    systemInstruction: CONSENT_PROMPT,
});
...
async function chatWithHistory(model, userMessage, history = []) {
...
async function askEmpathy(userMessage, history = []) {
    try {
        return await chatWithHistory(empathyModel, userMessage, history);
    } catch (error) {
        logger.error(`[Gemini/Empathy] Error: ${error.message}`);
        throw error;
    }
}
...
async function askDigDeeper(userMessage, history = []) {
    try {
        return await chatWithHistory(digDeeperModel, userMessage, history);
    } catch (error) {
        logger.error(`[Gemini/DigDeeper] Error: ${error.message}`);
        throw error;
    }
}
...
async function askOfferConsultation(userMessage, history = []) {
    try {
        const reply = await chatWithHistory(offerModel, userMessage, history);
        return reply + '\n\n✅ Готовы заполнить краткую анкету для записи на *бесплатную* консультацию?\nНапишите *Да* или *Нет*';
    } catch (error) {
        logger.error(`[Gemini/Offer] Error: ${error.message}`);
        throw error;
    }
}
...
async function generateProblemSummary(history = [], formData = {}) {
    try {
        ...
        const prompt = `Диалог с клиентом:\n${historyText}${formSection}\n\nОписание для юриста:`;
...
Определи: согласился ли клиент записаться на консультацию? (YES/NO)`;

        const result = await consentModel.generateContent(prompt);
        return result.response.text().trim().toUpperCase() === 'YES';
    } catch (error) {
        logger.error(`[Gemini/ConsentDetect] Error: ${error.message}`);
        return false;
    }
}
...
async function askGemini(userMessage, history = [], contextHint = '') {
    try {
        const fullMessage = contextHint
            ...
            : userMessage;
        const chat = scriptModel.startChat({ history: formatHistory(history) });
        const result = await chat.sendMessage(fullMessage);
        return result.response.text();
    } catch (error) {
        logger.error(`[Gemini/Script] Error: ${error.message}`);
        throw error;
    }
}
